...
def _build_flags_for_row(row):
    ...
    if pii_score > 0:
    # Prefer detailed info if available
        pii_detailed = row.get("pii_detailed")
        if isinstance(pii_detailed, str):
            try:
                pii_detailed = ast.literal_eval(pii_detailed)
            except Exception:
                pii_detailed = []
        if isinstance(pii_detailed, list):
            for entry in pii_detailed:
                origin = entry.get("origin_col", "unknown")
